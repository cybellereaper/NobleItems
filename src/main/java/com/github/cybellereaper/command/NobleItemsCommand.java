...
public final class NobleItemsCommand implements TabExecutor {
    ...
    public NobleItemsCommand(NobleItems plugin, CustomItemRegistry itemRegistry, CustomMobRegistry mobRegistry,
                             ItemFactory itemFactory, MobSpawner mobSpawner) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage(Component.text("Usage: /" + label + " reload|give|spawn"));
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            completions.addAll(List.of("reload", "give", "spawn"));
